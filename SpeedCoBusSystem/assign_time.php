<?php
 ...
 <form action="includes/assign_time.inc.php" method="GET">
 	...
 	<?php 
 		...
 		for($i=0;$i<$buses_size;$i++){
 			...
 			if(empty($current_employee['assigned_time'])){
 	 ...
 	 <?php 
 	 	...
 	 	$time = getObjects($connection,$sql);
 	 	$time_size = getObjectsSize($connection,$sql);

 	 	for($i=0;$i<$time_size;$i++){
 	 		$current_time = mysqli_fetch_assoc($time);

 	 		echo " <input type='checkbox' name='time[]' value=".$current_time['id'].">".$current_time['time']." - [ ID: ".$current_time['id']." ]<br>";
...
<a href="includes/group_remove_time.inc.php"></a>
 ...
 <?php 
 	...
 	for($i=0;$i<$_buses_size;$i++){
 		...
 		if(!empty($current__bus['assigned_time'])){

 			$shared_time = $current__bus['assigned_time'];
 			
 			#if they are not printed before, find all their group members and print them out
 			if(!in_array($shared_time,$time)){

 				#find all their group members
 				$sql = "SELECT * FROM buses WHERE assigned_time='$shared_time'";
 				$shared__buses = getObjects($connection,$sql);
 				$shared_size = getObjectsSize($connection,$sql);

 				echo "<table><tr><th>Registration</th><th>Time ID</th></tr>";
 				for($j=0;$j<$shared_size;$j++){
 					$current_shared__bus = mysqli_fetch_assoc($shared__buses);
 					echo "<tr><th>".$current_shared__bus['registration']."</th><th>".$current_shared__bus['assigned_time']."</th><th><a href='includes/group_remove_time.inc.php?id=".$current_shared__bus['id']."'>Remove</a></th></tr>";

 				}
 				echo "</table>";
 				array_push($time, $shared_time);
 			}
